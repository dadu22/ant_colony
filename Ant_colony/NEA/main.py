# Imported modules (Lines 2-9) - Imports necessary modules including Pygame, custom files, and thread executor
import pygame
...
# Executor for threading (Lines 11-12) - Initializes a thread pool executor for map generation
executor = ThreadPoolExecutor(max_workers=4)

pygame.init()
pygame.mixer.init()

# Variables for options and simulation screens (Lines 16-19) - Sets initial states for simulation and options menu
Sim_active = False
Options_menu = False
...
# Quadtree setup (Lines 22-33) - Configures quadtrees for mobs, pheromones, and spatial partitioning based on octaves
if octaves < 30:
...
# Variables needed for mobs, pheromones, and nest placement (Lines 36-58) - Defines initial counts, timers, and groups for game entities
ant_limit = 100
...
# Function to check if there is food on the screen and also kills red pheromones if no longer needed (Lines 61-71) - Checks for food and removes red pheromones when none remain
...
# Function to reset simulation to go back to options menu (Lines 74-100) - Resets entity counts, positions, and quadtrees for new simulation
...
# Simulation loop (Lines 103-111) - Begins main game loop and displays FPS
while True:
    ...
    for event in pygame.event.get():
        ...
        # Event handling for simulation and options menu setup (Lines 114-157) - Sets up event handling for menu navigation and button interactions
        if not Options_menu:
        ...
        if Options_menu:
            ...
            # Ant and food sliders (Lines 160-171) - Handles dragging for ant and food sliders
            ...
            # Seed input (Lines 174-182) - Handles seed input via keyboard
        ...
        # Event handling for simulation (Lines 185-227) - Manages simulation-specific events including nest placement and mob spawning
        if Sim_active:
    ...
    # Simulation screen rendering (Lines 230-284) - Manages quadtree updates, entity interactions, and screen drawing
    if Sim_active:
    ...
    # Options menu rendering (Lines 287-364) - Handles map generation, button rendering, and UI elements
    elif Options_menu:
    ...
    # Initial screen rendering (Lines 367-409) - Displays initial screen with title and decorative ants
    else:
        screen.blit(main_surface, (0, 0))
    ...
    # Frame update (Lines 412-414) - Updates display and controls frame rate
