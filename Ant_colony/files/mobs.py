# Imports and initialization (Lines 2-12) - Imports necessary modules and initializes Pygame and a thread pool executor
import pygame
import random
import numpy as np
...
# Ant class setup (Lines 14-53) - Defines the Ant class with initialization of sprite properties, image scaling, and movement attributes
...
# Ant image rotation method (Lines 55-60) - Rotates the ant's image based on direction
...
# Pheromone detection method (Lines 62-77) - Detects nearby pheromones using a quadtree query, filters by color
...
# Pheromone following method (Lines 79-150) - Implements logic for ants to follow pheromones or food, adjusting direction
...
# Angle calculation methods (Lines 152-161) - Calculates angle differences for pheromone following and movement
...
# Spider fleeing method (Lines 163-183) - Detects nearby spiders and calculates an escape angle for the ant
...
# Spider attraction method (Lines 185-190) - Calculates angle for ant to move towards a spider
    def move_towards_spider(self, spider_pos):
        dx = spider_pos[0] - self.rect.centerx
        dy = spider_pos[1] - self.rect.centery
        return np.arctan2(-dy, dx)

# Ant movement logic (Lines 192-256) - Handles ant movement, including terrain collision, spider interactions, and pheromone following
    def movement(self):
...
# Ant collision with boundaries (Lines 258-272) - Adjusts ant direction when hitting screen boundaries
    def collision(self):
        if self.rect.x < 5:
...
# Angle calculation helper methods (Lines 274-284) - Calculates direction and angle differences for boundary collisions
...
# Collision detection method (Lines 286-301) - Detects collision direction between ants
...
# Ant-to-ant collision handling (Lines 303-324) - Adjusts angle when ants collide based on collision direction
...
# Terrain collision handling (Lines 326-389) - Adjusts ant movement when colliding with terrain obstacles
...
# Food position update methods (Lines 391-398) - Updates food position if carried and provides ant position
...
# Ant update method (Lines 400-402) - Calls the movement method to update ant state
    def update(self):
        self.movement()

# Spider class setup (Lines 404-433) - Defines the Spider class with initialization of sprite properties and movement attributes
...
# Spider image rotation method (Lines 435-440) - Rotates the spider's image based on direction
    def rotate_image(self, ant_direction):
        rotated_image = pygame.transform.rotate(self.original_image, ant_direction - 90)
        self.image = rotated_image
        self.rect = self.image.get_rect(center=self.rect.center)

# Spider chase logic (Lines 442-465) - Implements spider behavior to chase nearby ants
    def chase_ants(self):
        vision_radius = 60
        vision_rect = pygame.Rect(
            self.rect.centerx - vision_radius,
...
# Spider movement logic (Lines 467-492) - Handles spider movement, including chasing and random wandering
    def movement(self):
        if self.eating:
            self.eating_timer -= 1
...
# Spider collision with boundaries (Lines 494-508) - Adjusts spider direction when hitting screen boundaries
    def collision(self):
        if self.rect.x < 5:
            self.angle += self.angle_direction(-0.25, 0.25) * 0.1
        elif self.rect.x >= WIDTH - 30:
...
# Spider angle calculation methods (Lines 510-520) - Calculates direction and angle differences for boundary collisions
    def angle_direction(self, start, end):
        angle_dir = random.uniform(start * np.pi, end * np.pi)
        angle_direct = self.angle_different(angle_dir)
        self.rotate_image(angle_direct)
...
# Spider terrain collision handling (Lines 522-585) - Adjusts spider movement when colliding with terrain obstacles
    def terrain_collision(self):
        if not self.is_alive:
            return
        self.pseudo_rect = self.rect.inflate(8, 8)
...
# Spider ant interaction (Lines 587-614) - Handles spider eating ants and death by swarm
        ...
        for ant in nearby_ants:
            if isinstance(ant, Ant) and ant.is_alive and self.rect.colliderect(ant.rect):
                if ant.holding_food is not None:
                    ant.holding_food.carried_by = None
                ant.kill()
                self.eating = True
                self.eating_timer = 60
                self.target_ant = ant
                break

# Spider update method (Lines 616-618) - Calls the movement method to update spider state
    def update(self):
        self.movement()

# Food class setup (Lines 620-637) - Defines the Food class with initialization and scaling based on octaves
...
# Food update and draw methods (Lines 639-646) - Updates food position if carried and draws it on the screen
    def update(self):
...
# Food spawning function (Lines 648-688) - Spawns clusters of food on valid terrain
